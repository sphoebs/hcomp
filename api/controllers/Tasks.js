      ...
      case recentTasksByAssignments:
        pool.connect((err, client, done) => {
          if (err) {
            ...
            const query = `SELECT t.* FROM tasks AS t INNER JOIN (SELECT min(id_task) as id_task,max("createdAt")  FROM assignments GROUP BY id_task) AS a ON t.id=a.id_task AND t.is_public=true AND t.is_live=true LIMIT 4`;
